(function(window,$){
  ...
  function addHighlightClass() {
    $('pre').addClass('highlight').each(function(){
      if ($(this).find('code.js').length > 0) {
        $(this).addClass('javascript').prepend('<div class="ribbon">');
      }
    });
  }
  function setPage() {
  ...
  function loadPageContent(state) {
    ...
    if (src === "false" || src === undefined) {
      ...
      $("[role=main]").load("/"+url+ " #documentation", function(){
        ...
        $("#list div").animate({ scrollTop: currentOffset + scrollTo - 122});
        if (method) {
          scrollDocsTo(method);
        }
      });
